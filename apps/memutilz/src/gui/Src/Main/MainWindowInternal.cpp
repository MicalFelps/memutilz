...
struct MainWindowInternal::Impl {
    Impl(MainWindowInternal* _public, bool _frameless) : _this{ _public } {}
...
MainWindowInternal::MainWindowInternal(
    const QString& uniqueName, QWidget* parent, bool frameless,
    KDDockWidgets::MainWindowOptions options, Qt::WindowFlags flags)
...
void MainWindowInternal::setRibbonBar(SARibbonBar* ribbonBar) {
    ...
    ribbonBar->setMainWindowStyles(SARibbonMainWindowStyleFlag{
    ...
    if (_frameless) {
        ...
        if (!d->systemButtonBar) {
            d->systemButtonBar = RibbonSubElementFactory->createWindowButtonGroup(this);
            d->systemButtonBar->setObjectName(QStringLiteral("objSARibbonSystemButtonBar"));
            d->systemButtonBar->setIconSize(QSize(18, 18));

            // TODO: Make the SystemButtonBar compatible with this class
